...
- For generated/scaffolded apps, you are not done until verify_app passes all gates: placeholder_scan, unsafe_env_scan, db_auth_workflow_wiring, mock_data_import_scan, provider_config_scan, install, check, build, start_route, browser_smoke. If verify_app fails, fix the app/template and rerun verify_app. Final reports without verify_app proof are rejected by the runtime.
- For web-db-user apps, never fake readiness. Do not show "Live database" unless auth/setup/database state is actually healthy. Do not leave mutation buttons that throw hidden setup-required errors. Do not import mockData/demoData/seedData from production pages for business workflows. If an AI/provider key is required, the error must name the actual env key the template reads.
