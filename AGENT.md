...
- Credential rule: before reporting missing deployment/API/model access, run `/home/toby/.local/bin/ccc doctor jeriko` and use `/home/toby/.local/bin/ccc run jeriko -- <command>` when a command needs stored credentials. Treat Vercel, GitHub, provider, and daemon auth as separate checks; if one fails, report that exact failed check and continue through any valid path.
- When building apps: load `operator-build-discipline`, `app-builder-production-sites`, and `premium-ui-motion` with `use_skill` before editing, then scaffold → write actual code → install dependencies → check/build → run verify_app → checkpoint → persistent local preview → report. NEVER just scaffold and stop.
- Premium contractor/local-service marketing sites: load `operator-build-discipline`, `app-builder-production-sites`, `premium-ui-motion`, and `contractor-site-autonomous-build`, then use `jeriko create from-prompt "<full user prompt>" --name "<business/site name>" --dir <project-dir>` so Jeriko selects the premium `web-static` contract automatically. Do not downgrade these sites to brochureware. Preserve or improve the premium system modules in the template: `LeadOpsVisual`, `LeadFlowLineSection`, `LeadLeakAudit`, `BeforeAfterComparison`, `StickyAuditRail`, SPA-safe `AppLink`/`useLocation` navigation, dark no-flash base styles, Vercel `dist/public` static routing, and concrete pricing/service lanes. Final proof for contractor/local-service marketing sites must include `premium_marketing_site_scan` passing and route-breadth proof for the requested pages. Primary nav must include visible Home, Services, Service Areas, Process, About, and Contact/Request Estimate items; logo-only Home does not count, and `Cities` must not replace `Service Areas`. Never use Toby/Hermes/operator voice in customer-facing copy: no `Badass`, `operator`, `mission`, `no excuses`, `trash`, `garbage`, `AI-built`, `generated`, or app-builder commentary. For OKC-area/remodeling local pages, use clear real city/community labels (Oklahoma City, Edmond, Yukon, Moore, Midwest City, Del City, Bethany, Warr Acres, etc.) and explain any limited radius in human copy; do not invent thin partial-city SEO pages like South Edmond/East Yukon. Service-area and city pages must have useful homeowner content, not formulaic “checked by scope/schedule/service radius/travel radius” filler. Preserve liked premium UI direction instead of flattening it into plain text blocks; fix cramped/glued UI labels before reporting done.
- Public website copy must speak as the business, not as the builder describing the website. Never write customer-visible copy like "the site speaks to...", "this site directs visitors...", "Cody's current site says...", "representative listings", or similar meta/build/rebuild language. Never expose SEO/builder production metrics in public UI: no "Guide depth", word-count badges, "1033 words", generated-content counts, content-depth labels, section-count claims, or app-builder commentary. Rewrite those as direct business copy about the service, customer, offer, property, route, or contact action.
...
- For generated/scaffolded apps, you are not done until verify_app passes all gates: placeholder_scan, scaffold_residue_scan, public_builder_meta_scan, unsafe_env_scan, primary_persistence_scan, db_auth_workflow_wiring, mock_data_import_scan, provider_config_scan, image_uniqueness_scan, install/check/test/build, production_artifact_scan, crawler_html, start_route, and browser_smoke. If verify_app fails, fix the app/template and rerun verify_app. Final reports without verify_app proof are rejected by the runtime.
- For web-db-user apps, never fake readiness. Do not show "Live database", "production ready", "AI connected", "real AI", or similar readiness claims unless the exact workflow has been live-proven by a route/API call or browser action in the current run. Do not leave mutation buttons that throw hidden setup-required errors. Do not import mockData/demoData/seedData from production pages for business workflows. If auth/database/provider setup is missing, the UI must show a clear setup-required state before the user can trigger the broken workflow.
- AI workflow rule: an env var existing is not proof that AI is connected. Trace the visible UI button → client mutation/fetch → server route → provider adapter → model/API response. If the workflow needs auth/database, either wire a visible sign-in/setup path or make the AI action public/read-only and keep persistence actions gated. Prove live AI with a minimal server call or browser click that returns model-generated content; otherwise say setup is missing and name the required env key.
- Browser workflow rule: for every primary visible action button (scan, save, add, create, upload, generate, order, checkout), prove it changes URL, DOM, page text, durable state, or server data. If it cannot run because setup is missing, disable it or show a visible setup-required explanation instead of letting it silently fail.
- No-progress rule: after a failed verify_app gate, do not repeatedly call workspace_status or reread the same files. The next distinct action must be to fix the named failed gate, run the narrow verification, or report the exact blocker.
...
- For SEO/service/city/location pages, separate content into real visible sections such as Overview, Local context, What we review, Common project risks, Process, Pricing/next step, FAQ, and Related areas/services. Multi-route sites must keep page structure maintainable: use route-specific page components or clearly named section components, not one giant Home.tsx full of unrelated one-line page functions.
